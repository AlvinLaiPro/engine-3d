...
export default function resl(config) {
  ...
  function loadElement(request, element) {
    ...
    } else if (request.type === 'wxgameaudio') {
      element.onCanplay(onComplete);
    } else {
  ...
  let loaders = {
    ...
    wxgameaudio: function (request) {
      return loadElement(request, wx.createInnerAudioContext());
    }
  };
